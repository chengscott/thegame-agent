...
class Client(GuiClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        h_dis = []
        b_dis = []
        upgrade_list = [Ability.HealthRegen,Ability.MaxHealth,Ability.BodyDamage,Ability.BulletSpeed]
        ...
        if heroes:
            for i in range(len(heroes)):
                ...
                h_dis.append(d)
            ## 如果距離過近，則用反方向遠離
            if min(h_dis) < 500 and hero.level < heroes[h_dis.index(min(h_dis))].level:
            ...
            elif min(h_dis) < 500 and hero.level > heroes[h_dis.index(min(h_dis))].level and hero.level > 30:
                ex, ey = heroes[h_dis.index(min(h_dis))].position
                cx, cy = heroes[h_dis.index(min(h_dis))].velocity
