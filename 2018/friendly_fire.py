...
class Client(HeadlessClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        def move():
            ...
            for h in heroes:
                w = heroes.index(h)
                if dis(hero.position, h.position) < dis(
                        hero.position, heroes[z].position):
                    z = w
            x1, y1 = hero.position
            ...
            if hero.level < 17:
                if abs(x1 - x2) <= 100 and abs(y1 - y2) <= 350:
                    ...
                    self.accelerate_towards(
                        x2 - x1, float(random.choice(['800', '-800'])))
                elif abs(x1 - x2) and not abs(y1 - y2) <= 350:
                    self.accelerate_towards(
                        float(random.choice(['800', '-800'])), y2 - y1)
                elif abs(x1 - x2) <= 300 and abs(x1 - x2) <= 300:
                    self.accelerate_towards(-x2 + random.randint(-100, 100),
        ...
        if polygons:
            if hero.level < 9:
                ...
                for p in polygons:
                    ...
                    if exp < ex and sqrt(
                            dis(hero.position,
                                p.position)) >= hero.bullet_speed * 2:
                        cont = p1
                ...
                if not heroes:
                    ...
                    for p in polygons:
                        p1 = polygons.index(p)
                        ex = p.rewarding_experience
                        if exp > ex and (p.health < polygons[cont].health or
                                         sqrt(dis(hero.position, p.position))
                                         >= hero.bullet_speed * 3):
                            cont = p1
                    x1, y1 = hero.position
                    x2, y2 = polygons[cont].position
            ...
            self.accelerate_towards(
                random.randint(-800, 800), random.randint(-800, 800))
