    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        def strong():
             for i in range (len(heroes)):
                 if hero.level-heroes[i].level<=0 and dist(*heroes[i].position)<1500:
        ...
        def kill():
            for i in range (len(heroes)):
                if heroes[i].health/heroes[i].abilities.max_health.value<0.1 and dist(*heroes[i].position)<300:
                      if self.gg>(heroes[i].health/heroes[i].abilities.max_health.value):
                         self.gg=(heroes[i].health/heroes[i].abilities.max_health.value)
                         self.ap=i
            if  self.ap==0:
                 return False
            else:
        ...
        if heroes and kill():
        ...
        elif heroes and dist(*heroes[l].position)<=300 and hero.level-heroes[l].level>=2 and hero.health/hero.abilities.max_health.value>=0.4:
            ...
            if polygons and dist(*polygons[h].position)<dist(*heroes[self.s].position):
