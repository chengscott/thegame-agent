...
def win_hero(h, e):
    return h.health/(e.body_damage+e.bullet_damage) > e.health/(h.body_damage+h.bullet_damage)
...
def arc(h, e):
    if h.position[1] == e.position[1]:
        ...
        return 720 * math.pi / math.atan(e.position[1]-h.position[1])/(e.position[0]-h.position[0])
    else:
        return math.pi*2 - 720 * math.pi / math.atan(e.position[1]-h.position[1])/(e.position[0]-h.position[0])
...
class Client(GuiClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        if bullets:
            enemy = None
            for h in heroes:
                if h.name == enemy_name or destination(hero, h) < hero.radius:
                    enemy = h
                    break
            if not enemy:
                for b in bullets:
                    if destination(hero, b) < hero.radius:
                        enemy = heroes[0]
                        for h in heroes:
                            if destination(hero, h) < enemy:
                                enemy = h
            if enemy:
                if win_hero(hero, enemy):
                    self.accelerate(arc(hero, enemy))
                else:
                    self.accelerate((arc(hero, enemy)+math.pi/2) % (math.pi*2))
        ...
        if heroes and not is_moving:
            ...
            for h in heroes:
                if win_hero(hero, h):
                    if (not loser) or h.rewarding_experience > loser.rewarding_experience:
                        loser = h
            if loser:
                self.accelerate_towards(*loser.position)
                self.shoot_at(*loser.position)
                print('loser(x, y): ', int(loser.position[0]), int(loser.position[1]), 'health: ', loser.health, hero.health)
                is_moving = True
        # Polygons around
            ...
            for p in polygons:
                if win_polygon(hero, p):
                    if (not target) or p.rewarding_experience > target.rewarding_experience:
                        target = p
            if target:
                ...
                if destination(hero, polygons[0]) < 1.5*hero.radius: self.accelerate_towards(*polygons[0].position)
