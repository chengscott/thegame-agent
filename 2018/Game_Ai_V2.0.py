    ...
    #控制射擊目標
        ...
        if flag_enemynear == True and heroes:
            ...
            if (((x - xfoe)**2 + (y - yfoe)**2)**0.5) < 300:
                self.accelerate_towards(xfoe + (x - foe) * 2,
                                        yfoe + (y - yfoe) * 2)

        if flag_bulletnear == True and bullets:
            self.accelerate_towards(x + (y - ybullet) * (-1),
                                    y + (x - xbullet))

        if flag_blocknear == True and polygons:
            self.accelerate_towards(xpoly + (x - xpoly) * 2,
                                    ypoly + (y - ypoly) * 2)

        if flag_bossnear == True and heroes:
            self.accelerate_towards(xboss + (x - xboss) * 4,
                                    yboss + (y - yboss) * 4)
            if flag_blocknear == True:
                self.accelerate_towards(x + (y - ypoly) * (-1),
                                        y + (x - xpoly))
