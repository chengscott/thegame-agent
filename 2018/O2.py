...
class Client(GuiClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        if polygons:
            ...
            elif hero.abilities.bullet_speed.level >=6 and distance(hero.position, polygons[i].position)<50000 and hero.health_regen_level>50:
                self.accelerate_towards(*polygons[i].position)
            else:
        ...
        if heroes:
            ...
            if (distance(hero.position, heroes[t].position)>50000 and hero.health_regen_level>50) or hero.level>heroes[t].level:
