        ...
        if self.run :
            ...
            if heroes :
                if self.run > 400 :
                    ...
                    self.accelerate( rad(heroes[0].position[0] - hero.position[0], heroes[0].position[1] - hero.position[1]) + math.pi )
                    self.shoot_at( *heroes[0].position )
            else :
                run = 0
            ...
            for hr in heroes :
                if ( hr.id == self.atkID ) :
                    ...
                    self.shoot( rad ( hr.position[0] - hero.position[0], hr.position[1] - hero.position[1] ) + math.pi )
        else :
            self.atkID = int(-1e9)
            for hr in heroes :
                if get_dis( *hr.position ) < 8000 and hr.level - hero.level > 3 :
                    self.run = 1
                    self.stpl = rad(heroes[0].position[0] - hero.position[0], heroes[0].position[1] - hero.position[1])
                    break
            if( hero.abilities.health_regen.level == 8 and hero.abilities.max_health.level == 8 ) :
                if heroes and get_dis(*heroes[0].position) < 5000:
                    idx = 0
                    print("Win")
                    self.accelerate_towards( heroes[idx].position[0], heroes[idx].position[1]  )
                    self.shoot( rad ( heroes[idx].position[0] - hero.position[0], heroes[idx].position[1] - hero.position[1] ) + math.pi )
                    if heroes[0].health < ( heroes[0].max_health / 3 ) :
                        self.atkID = heroes[0].id
                elif polygons :
                    idx = 0
                    if( self.clk % 100 < 50 ):
                        if( get_dis( *polygons[idx].position ) > 3000 and polygons[idx].max_health < 200 ):
                            self.accelerate_towards( *polygons[idx].position )
                    if polygons[idx].max_health >= 200 :
                        polygons.sort( key=cmp_to_key(cm_mhp) )
                    self.shoot_at( polygons[idx].position[0] , polygons[idx].position[1] )
            else :
                if heroes and get_dis(*heroes[0].position) < 1000 and heroes[0].level - hero.level < 3 :
                    idx = 0
                    self.accelerate_towards( heroes[idx].position[0] , heroes[idx].position[1] )
                    self.shoot_at( heroes[idx].position[0] + random.uniform(-1, 1), heroes[idx].position[1] + random.uniform(-1, 1), )
                elif polygons :
                    idx = 0
                    if polygons[idx].max_health >= 200 :
                        polygons.sort( key=cmp_to_key(cm_mhp) )
