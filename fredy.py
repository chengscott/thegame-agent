...
class Client(GuiClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        for j in range(0,len(polygons)):
            if polygons:
                dist.append((polygons[j].position.x-hero.position.x)**2+(polygons[j].position.y-hero.position.y)**2)
        for k in range(0,len(heroes)):
            if heroes:
                dist2.append((heroes[k].position.x-hero.position.x)**2+(heroes[k].position.y-hero.position.y)**2)
        ...
        print("-" * 79)
        if polygons:
            dis=(polygons[a].position.x-hero.position.x)**2+(polygons[a].position.y-hero.position.y)**2
        if heroes:
            dis2=(heroes[b].position.x-hero.position.x)**2+(heroes[b].position.y-hero.position.y)**2
        if dis2<20*hero.radius**2 and heroes:
            self.shoot_at(*heroes[b].position)
            self.accelerate_towards(2*hero.position.x-2*heroes[b].position.x,2*hero.position.y-2*heroes[b].position.y)
        elif dis<50*hero.radius**2 and polygons:
            self.accelerate_towards(hero.position.x-polygons[a].position.x,hero.position.y-polygons[a].position.y)
