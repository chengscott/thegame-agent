        ...
        if hero.abilities.health_regen.level <=4 or hero.abilities.max_health.level <=3 or hero.abilities.body_damage.level<=4 or hero.abilities.reload.level<=3:    #能力不足
            ...
            if heroes:
                for l in range (0,len(heroes)):
                    if ((heroes[l].position[0]-hero.position[0])**2+(heroes[l].position[1]-hero.position[1])**2)**0.5<= ((heroes[0].position[0]-hero.position[0])**2+(heroes[0].position[1]-hero.position[1])**2)**0.5 :
                        heroes[0]=heroes[l]
                          #躲敵人
                        ...
                        if ((hero.position[0]-heroes[0].position[0])**2+(hero.position[1]-heroes[0].position[1])**2)< ((X2P3 - X43)**2 + (Y2N3 - Y43)**2):
            ...
            if polygons:
                for j in range (1,len(polygons)):
                    if ((polygons[j].position[0]-hero.position[0])**2+(polygons[j].position[1]-hero.position[1])**2)<=((polygons[0].position[0]-hero.position[0])**2+(polygons[0].position[1]-hero.position[1])**2):
                        ...
                        if ((hero.position[0]-polygons[0].position[0])**2+(hero.position[1]-polygons[0].position[1])**2)< ((X2P - X4)**2 + (Y2N - Y4)**2):
            ...
            if bullets:
                for i in range (1,len(bullets)):
                    if ((bullets[i].position[0]-hero.position[0])**2+(bullets[i].position[1]-hero.position[1])**2)**0.5<((bullets[0].position[0]-hero.position[0])**2+(bullets[0].position[1]-hero.position[1])**2)**0.5 and bullets[i].owner_id!=self.name:
                        bullets[0]=bullets[i]
                        ...
                        if ((hero.position[0]-bullets[0].position[0])**2+(hero.position[1]-bullets[0].position[1])**2)< ((X2P2 - X42)**2 + (Y2N2 - Y42)**2):
            ...
            if heroes:
                ...
                for j in range (1,len(polygons)):
                    if ((polygons[j].position[0]-hero.position[0])**2+(polygons[j].position[1]-hero.position[1])**2)<((polygons[0].position[0]-hero.position[0])**2+(polygons[0].position[1]-hero.position[1])**2):
                        ...
                        self.accelerate_towards(polygons[0].position[0],polygons[0].position[1])
                        self.shoot_at(*polygons[0].position)
