...
class Client(GuiClient):
        ...
        for a in range(0,len(bullets)):
            if bullets[a].owner_id != hero.name and bullets[a].body_damage > 10 :
                ...
                for j in range(0,len(bullets)):
                    bulldis[j] = ((bullets[j].position[0] - hero.position[0])**2 + (bullets[j].position[1] - hero.position[1])**2)**0.5
                for k in range(0,len(bullets)):
                    if bulldis[k] <= 400:
                        bulldisapp.append(k)
        for b in range(0,len(bulldisapp)):
            self.accelerate_towards(hero.position[0] + (hero.position[0] - bullets[k].position[0]),hero.position[1] + (hero.position[1] - bullets[k].position[1]))
    ...
    def polygons_(self, hero, heroes, polygons, bullets):
        ...
        for j in range(0,len(polygons)):
            polydis[j] = ((polygons[j].position[0] - hero.position[0])**2 + (polygons[j].position[1] - hero.position[1])**2)**0.5
        ...
        for m in range(0,len(heroes)):
            enemydis[m] = ((heroes[m].position[0] - hero.position[0])**2 + (heroes[m].position[1] - hero.position[1])**2)**0.5
    ...
    def enemy_(self, hero, heroes, polygons, bullets):
        ...
        if minenemyhealth >= 30 and minenemyhealth >= 30:
            ...
            if (maxenemylv - hero.level) >= 5:
                self.accelerate_towards(hero.position[0] + (hero.position[0] - heroes[r].position[0]),hero.position[1] + (hero.position[1] - heroes[r].position[1]))
