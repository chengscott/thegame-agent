    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        if polygons:
            ...
            for j1 in range(0, len(polygons)):
                polydis[j1] = (
                    (polygons[j1].position[0] - hero.position[0])**2 +
                    (polygons[j1].position[1] - hero.position[1])**2)**0.5
                print(polydis)
                if len(polydis) >= 0:
            ...
            for y1 in range(0, len(heroes)):
                enemydis[y1] = (
                    (heroes[y1].position[0] - hero.position[0])**2 +
                    (heroes[y1].position[1] - hero.position[1])**2)**0.5
                if len(enemydis) >= 0:
            ...
            if heroes and enemydis[z1] - polydis[k1] <= 1000:
                ...
                if minenemydis >= 100 and enemylv[r] - hero.level < 1 and hero.level > 5:
                    self.accelerate_towards(heroes[z1].position[0],
                                            heroes[z1].position[1])
                    self.shoot_at((heroes[z1].position[0]),
                                  (heroes[z1].position[1]))
            ...
            if polygons:
                ...
                if minpolydis >= 150:
                    self.accelerate_towards((polygons[k1].position[0]) + x,
                                            (polygons[k1].position[1]) + x)
                    self.shoot_at(polygons[k1].position[0],
                                  polygons[k1].position[1])
                if minpolydis <= 150:
                    self.accelerate_towards(-(polygons[k1].position[0]),
                                            -(polygons[k1].position[1]))
                    self.shoot_at(polygons[k1].position[0],
                                  polygons[k1].position[1])
