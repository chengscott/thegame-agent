    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        if polygons:
            ...
            for i in range(0, len(polygons)):
                poly_dist = (
                    (polygons[i].position[0] - hero.position[0])**2 +
                    (polygons[i].position[1] - hero.position[1])**2)**0.5
        ...
        if heroes:
            ...
            for i in range(0, len(heroes)):
                hero_dist = ((heroes[i].position[0] - hero.position[0])**2 + (
                    heroes[i].position[1] - hero.position[1])**2)**0.5
            ...
            if hero.health >= target_hero.health and hero.level >= target_hero.level and hero.health / hero.max_health >= 0.2:
                ...
                self.accelerate_towards(
                    2 * hero.position[0] - target_hero.position[0],
                    2 * hero.position[1] - target_hero.position[1])
                self.shoot_at(*target_hero.position)
        if polygons:
            if poly.edges == 5 and (
                (poly.position[0] - hero.position[0])**2 +
                (poly.position[1] - hero.position[1])**2)**0.5 <= 200:
                posx = 1.5 * hero.position[0] - 0.5 * poly.position[0]
                posy = 1.5 * hero.position[1] - 0.5 * poly.position[1]
                self.accelerate_towards(posx, posy)
