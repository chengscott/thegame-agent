...
class Client(HeadlessClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        if heroes:
            opp = 0
            d = (hero.position[0] - (heroes[0].position[0]))**2 + (
                hero.position[1] - (heroes[0].position[1]))**2
            for i in range(1, len(heroes)):
                if d > (hero.position[0] - heroes[i].position[0])**2 + (
                        hero.position[1] - heroes[i].position[1]):
                    d = (hero.position[0] - heroes[i].position[0])**2 + (
                        hero.position[1] - heroes[i].position[1])
                    opp = i
            if hero.health <= heroes[opp].health:
                self.accelerate_towards(
                    1.3 * (2 * hero.position[0] - heroes[opp].position[0]),
                    0.8 * (2 * hero.position[1] - heroes[opp].position[1]))
                ...
                self.shoot_at(*heroes[opp].position)
        if polygons:
            pd = (hero.position[0] - polygons[0].position[0])**2 + (
                hero.position[1] - polygons[0].position[1])**2
            ply = 0
            for i in range(1, len(polygons)):
                if pd > (hero.position[0] - polygons[i].position[0])**2 + (
                        hero.position[1] - polygons[i].position[1]):
                    pd = (hero.position[0] - polygons[i].position[0])**2 + (
                        hero.position[1] - polygons[i].position[1])
                    ply = i
