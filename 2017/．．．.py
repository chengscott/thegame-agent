...
class Client(GuiClient):
    ...
    def action(self, hero, heroes, polygons, bullets):
        ...
        for a in range (0,len(bullets)):
            if ((bullets[a].position[0]-hero.position[0])**2+(bullets[a].position[1]-hero.position[1])**2)**0.5<10:
                self.accelerate_towards(polygons[0].position[0],polygons[0].position[1])
                if ((bullets[a].position[0]-hero.position[0])**2+(bullets[a].position[1]-hero.position[1])**2)**0.5>=10:
                    break
            for i in range (0,len(heroes)):
                if ((heroes[i].position[0]-hero.position[0])**2+(heroes[i].position[1]-hero.position[1])**2)**0.5<10:
