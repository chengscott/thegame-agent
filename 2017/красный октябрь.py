        ...
        if len(pentagons) != 0:
            if hero.health > hero.health/4:
                for i in range (0,len(pentagons)):
                    if distance(pentagons[i],hero) == pentagons[i].radius+hero.radius + 90:
                        self.accelerate_towards(-pentagon.position[0],-pentagon.position[1])
                        if len(heroes)!= 0:
                            mindisH = minDistHero(hero,heroes)
                            if distance(hero,mindisH) < 10:
                                self.accelerate_towards(-mindisH.position[0],-mindisH.position[1])
                    else:
...
def distance (object1,object2):
    return math.sqrt((object1.position[0]-object2.position[0])**2+(object1.position[1]-object2.position[1])**2)
def center (polygons):
    res = ()
    xt = 0
    yt = 0
